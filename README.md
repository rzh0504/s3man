...
- 文件浏览 -- 文件夹导航、面包屑路径、搜索、排序、类型过滤和分页加载
- 文件操作 -- 上传、下载、删除、重命名、复制、移动文件/文件夹，支持批量操作
- 文件预览 -- 图片缩放查看、视频播放、代码/文本高亮、PDF 浏览器打开
- 传输管理 -- 真实进度展示、可配置并发数，下载支持暂停/恢复，上传/下载支持取消
- 分享上传 -- 从系统分享菜单直接上传文件到指定存储桶
- 链接分享 -- 生成预签名 URL 或通过 Worker 代理生成短链接
- 存储桶管理 -- 创建/删除存储桶，按连接分组展示
- 深色模式 -- 跟随系统或手动切换
- 多语言 -- 中文 / English
- Cloudflare Worker 代理 -- 可选部署，用于加速访问和生成自定义域名链接，应用内请求通过 Header 传递凭证
...
- 大桶浏览默认分页加载，进入文件夹时只加载首批对象，可手动继续加载更多。
- 超过 32MB 的上传会使用 S3 Multipart Upload，并对分片上传做有限重试。
- 正在进行的传输任务无法跨 App 重启继续执行；重启后会在传输历史中标记为中断失败，避免残留不可操作的进行中任务。
- 配置导出文件包含访问凭证，当前使用 AES-GCM 加密；仍应妥善保管导出的 `.s3man` 文件。
- Web 端凭证受浏览器本地存储安全边界限制，不建议在不受信任设备上保存长期凭证。
